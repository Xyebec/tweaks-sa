...
auto is_executable(uint8_t* address) -> bool {
...
auto UnprotectMemory::operator=(UnprotectMemory&& other) noexcept -> UnprotectMemory& {
    if (this != &other) {
...
auto unprotect(uint8_t* address, size_t size) -> std::optional<UnprotectMemory> {
