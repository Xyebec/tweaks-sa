...
auto vm_allocate(uint8_t* address, size_t size, VmAccess access) -> std::expected<uint8_t*, OsError> {
    DWORD protect = 0;
...
auto vm_protect(uint8_t* address, size_t size, VmAccess access) -> std::expected<uint32_t, OsError> {
    DWORD protect = 0;
...
auto vm_protect(uint8_t* address, size_t size, uint32_t access) -> std::expected<uint32_t, OsError> {
    DWORD old_protect = 0;

    if (VirtualProtect(address, size, access, &old_protect) == FALSE) {
        return std::unexpected{OsError::FAILED_TO_PROTECT};
    }
...
auto vm_query(uint8_t* address) -> std::expected<VmBasicInfo, OsError> {
...
auto vm_is_readable(uint8_t* address, size_t size) -> bool {
    return IsBadReadPtr(address, size) == FALSE;
}

auto vm_is_writable(uint8_t* address, size_t size) -> bool {
    return IsBadWritePtr(address, size) == FALSE;
}

auto vm_is_executable(uint8_t* address) -> bool {
...
public:
    ...
    auto find_trap(const uint8_t* address) -> TrapInfo* {
        auto search = std::ranges::find_if(m_traps, [address](auto& trap) {
    ...
    auto find_trap_page(const uint8_t* address) -> TrapInfo* {
        auto search = std::ranges::find_if(m_traps, [address](auto& trap) {
        ...
        search = std::ranges::find_if(m_traps, [address](auto& trap) {
...
private:
    ...
    static auto CALLBACK trap_handler(PEXCEPTION_POINTERS exp) -> LONG {
...
void fix_ip(ThreadContext thread_ctx, const uint8_t* old_ip, const uint8_t* new_ip) {
