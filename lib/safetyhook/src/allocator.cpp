...
auto Allocator::global() -> std::shared_ptr<Allocator> {
...
auto Allocator::create() -> std::shared_ptr<Allocator> {
    return std::shared_ptr<Allocator>{new Allocator{}};
}

auto Allocator::allocate(size_t size) -> std::expected<Allocation, Allocator::Error> {
    return allocate_near({}, size, std::numeric_limits<size_t>::max());
}

auto Allocator::allocate_near(const std::vector<uint8_t*>& desired_addresses, size_t size, size_t max_distance) -> std::expected<Allocation, Error> {
...
auto Allocator::internal_allocate_near(const std::vector<uint8_t*>& desired_addresses, size_t size, size_t max_distance) -> std::expected<Allocation, Error> {
...
auto Allocator::allocate_nearby_memory(const std::vector<uint8_t*>& desired_addresses, size_t size, size_t max_distance) -> std::expected<uint8_t*, Error> {
...
auto Allocator::in_range(const uint8_t* address, const std::vector<uint8_t*>& desired_addresses, size_t max_distance) -> bool {
