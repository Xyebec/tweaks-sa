...
auto VmHook::operator=(VmHook&& other) noexcept -> VmHook& {
    destroy();
...
auto VmtHook::create(void* object) -> std::expected<VmtHook, Error> {
    VmtHook hook{};

    auto *const original_vmt = *reinterpret_cast<uint8_t***>(object);
...
auto VmtHook::operator=(VmtHook&& other) noexcept -> VmtHook& {
    destroy();
...
void VmtHook::remove(void* object) {
    ...
    auto *const original_vmt = search->second;
