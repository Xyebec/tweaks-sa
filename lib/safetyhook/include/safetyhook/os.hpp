...
struct VmAccess {
    ...
    constexpr auto operator==(const VmAccess& other) const -> bool {
...
auto SAFETYHOOK_API vm_allocate(uint8_t* address, size_t size, VmAccess access) -> std::expected<uint8_t*, OsError>;
void SAFETYHOOK_API vm_free(uint8_t* address);
auto SAFETYHOOK_API vm_protect(uint8_t* address, size_t size, VmAccess access) -> std::expected<uint32_t, OsError>;
auto SAFETYHOOK_API vm_protect(uint8_t* address, size_t size, uint32_t access) -> std::expected<uint32_t, OsError>;
auto SAFETYHOOK_API vm_query(uint8_t* address) -> std::expected<VmBasicInfo, OsError>;
auto SAFETYHOOK_API vm_is_readable(uint8_t* address, size_t size) -> bool;
auto SAFETYHOOK_API vm_is_writable(uint8_t* address, size_t size) -> bool;
auto SAFETYHOOK_API vm_is_executable(uint8_t* address) -> bool;
...
auto SAFETYHOOK_API system_info() -> SystemInfo;
...
void SAFETYHOOK_API fix_ip(ThreadContext ctx, const uint8_t* old_ip, const uint8_t* new_ip);
