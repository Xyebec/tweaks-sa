...
auto SAFETYHOOK_API is_executable(uint8_t* address) -> bool;
...
public:
    ...
    auto operator=(const UnprotectMemory&) -> UnprotectMemory& = delete;
    auto operator=(UnprotectMemory&& other) noexcept -> UnprotectMemory&;

private:
    friend auto SAFETYHOOK_API unprotect(uint8_t*, size_t) -> std::optional<UnprotectMemory>;
...
[[nodiscard]] auto SAFETYHOOK_API unprotect(uint8_t* address, size_t size) -> std::optional<UnprotectMemory>;

template <typename T>
constexpr auto align_up(T address, size_t align) -> T {
...
template <typename T>
constexpr auto align_down(T address, size_t align) -> T {
    const auto unaligned_address = address_cast<uintptr_t>(address);
