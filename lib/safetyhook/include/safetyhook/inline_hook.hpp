...
public:
    ...
    struct Error {
        ...
        [[nodiscard]] static auto failed_to_decode_instruction(uint8_t* ip) -> Error {
            return Error{
                .type = FAILED_TO_DECODE_INSTRUCTION,
                .ip = ip,
            };
        }
        ...
        [[nodiscard]] static auto short_jump_in_trampoline(uint8_t* ip) -> Error {
            return Error{
                .type = SHORT_JUMP_IN_TRAMPOLINE,
                .ip = ip,
            };
        }
        ...
        [[nodiscard]] static auto ip_relative_instruction_out_of_range(uint8_t* ip) -> Error {
            return Error{
                .type = IP_RELATIVE_INSTRUCTION_OUT_OF_RANGE,
                .ip = ip,
            };
        }
        ...
        [[nodiscard]] static auto unsupported_instruction_in_trampoline(uint8_t* ip) -> Error {
            return Error{
                .type = UNSUPPORTED_INSTRUCTION_IN_TRAMPOLINE,
                .ip = ip,
            };
        }
        ...
        [[nodiscard]] static auto failed_to_unprotect(uint8_t* ip) -> Error {
            return Error{
                .type = FAILED_TO_UNPROTECT,
                .ip = ip,
            };
        }
        ...
        [[nodiscard]] static auto not_enough_space(uint8_t* ip) -> Error {
            return Error{
    ...
    [[nodiscard]] static auto create(void* target, void* destination, Flags flags = Default) -> std::expected<InlineHook, Error>;
    ...
    [[nodiscard]] static auto create(T target, U destination, Flags flags = Default) -> std::expected<InlineHook, Error> {
    ...
    [[nodiscard]] static auto create(const std::shared_ptr<Allocator>& allocator,
        void* target, void* destination, Flags flags = Default) -> std::expected<InlineHook, Error>;
    ...
    template <typename T, typename U>
    [[nodiscard]] static auto create(const std::shared_ptr<Allocator>& allocator,
        T target, U destination, Flags flags = Default) -> std::expected<InlineHook, Error> {
    ...
    auto operator=(InlineHook&& other) noexcept -> InlineHook&;
    ~InlineHook();
    ...
    [[nodiscard]] auto destination() const -> uint8_t* { return m_destination; }

    /// @brief Get the destination address.
    /// @return The destination address.
    [[nodiscard]] auto destination_address() const -> uintptr_t { return reinterpret_cast<uintptr_t>(m_destination); }

    /// @brief Get the trampoline Allocation.
    /// @return The trampoline Allocation.
    [[nodiscard]] auto trampoline() const -> const Allocation& { return m_trampoline; }
    ...
    template <typename T>
    [[nodiscard]] auto original() const -> T { return reinterpret_cast<T>(m_trampoline.address()); }
    ...
    [[nodiscard]] auto original_bytes() const -> const auto& { return m_original_bytes; }
    ...
    template <typename RetT = void, typename... Args>
    auto unsafe_call(Args... args) -> RetT {
    ...
    /// safety or are worried about the performance cost of locking the mutex.
    template <typename RetT = void, typename... Args>
    auto unsafe_ccall(Args... args) -> RetT {
    ...
    /// safety or are worried about the performance cost of locking the mutex.
    template <typename RetT = void, typename... Args>
    auto unsafe_thiscall(Args... args) -> RetT {
    ...
    /// safety or are worried about the performance cost of locking the mutex.
    template <typename RetT = void, typename... Args>
    auto unsafe_stdcall(Args... args) -> RetT {
    ...
    /// safety or are worried about the performance cost of locking the mutex.
    template <typename RetT = void, typename... Args>
    auto unsafe_fastcall(Args... args) -> RetT {
...
private:
    ...
    auto setup(const std::shared_ptr<Allocator>& allocator, uint8_t* target, uint8_t* destination) -> std::expected<void, Error>;
    auto e9_hook(const std::shared_ptr<Allocator>& allocator) -> std::expected<void, Error>;
